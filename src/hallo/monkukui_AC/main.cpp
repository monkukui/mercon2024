...
int main() {
    ...
    auto g = [](lint a, lint b) { return b; };
    SegmentTree<lint, lint, decltype(f), decltype(g)> sg(f, g, -INF);
    sg.init(200010);
    sg.build(vector<lint>(200010, -INF));

    sg.update(0 + 100000, 0); // 初期化

    // u が大きい順、v が小さい順
    vector<tuple<lint, lint, lint>> info; // {v, -u, c}
    for (int i = 0; i < n; i++) {
        info.emplace_back(v[i], -u[i],c[i]);
    }
    sort(ALL(info));

    for (auto [v, u, c] : info) {
        u = -u;
        lint max_value = sg.query(u, 200010);
