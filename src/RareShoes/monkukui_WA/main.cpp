...
int main() {
    ...
    vector<lint> x(n), y(n);
    vector<pair<lint, int>> info; // (時刻, -1 or 終点の時刻)
    for (int i = 0; i < n; i++) {
        cin >> x[i] >> y[i];
        info.emplace_back(x[i], y[i]);
        assert(0 <= x[i] and x[i] <= 1000000000);
        assert(0 <= y[i] and y[i] <= 1000000000);
    }
    vector<lint> z(m);
    for (int i = 0; i < m; i++) {
        cin >> z[i];
        info.emplace_back(z[i], -1);
        assert(0 <= z[i] and z[i] <= 1000000000);
    }

    sort(ALL(info));
    set<lint> st;
    int ans = 0;
    for (auto [t, type] : info) {
        if (type != -1) {
            st.insert(type);
        } else {
